...
from api.controller.product_controller import fetch_and_list_products_by_shopname
...
# @protect()
def deleteCoupon(
    product_id: str, product_collection: Collection = Depends(get_product_collection)
):
    return delete_product(product_id, product_collection)


@product_router.get("/fetch-products-by-shopname")
# @protect()
def fetchProductsByShop(
    shop_name: str = Query(...),
    product_collection: Collection = Depends(get_product_collection),
    shopkeeper_collection: Collection = Depends(get_shopkeeperdata_collection),
):
    return fetch_and_list_products_by_shopname(shop_name, product_collection, shopkeeper_collection)
