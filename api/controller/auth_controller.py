...
def login_user(user:UserLogin,response:Response,user_collection:Collection):
        ...
        if not db_user:
            ...
            raise HTTPException(status_code=401, detail="Incorrect email or password",headers=None)
        # Generate token
...
def get_current_user(request:Request,user_collection:Collection):
    ...
    print(cookie)
    if(not cookie):
        raise HTTPException(status_code=401,detail="Unauthorized access! Please login again")
    ...
    if user is None:
        raise credentials_exception
    user_data = {"email": user["email"], "role": user.get("role", "user")}  # Get role with default
    return {
        "status":"success",
        "isLoggedIn":True,
        "user":user_data
